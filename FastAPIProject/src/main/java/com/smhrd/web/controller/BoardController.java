...
@Controller // --> Spring Controller가 해당하는 파일을 scan 할수있게끔 해주는 annotation
// Application은 선언구문보다 위쪽에 작성하는것이 원칙이다
public class BoardController {
	
	//데이터 흐름을 체크할 수 있는 log 찍는 도구 꺼내오기
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	@Autowired // --> container에 자동으로 생성된 객체를 
	...
	// 정확한 전송방식을 지정하는 것을 권장
	 @GetMapping("/")
		...
		// 내가 설정할 prefix + board + suffix
		// * forward 방식으로 이동 : 요청 1번 , 응답 1번 
		// --> 초기 요청 url일 변하지 않고, 요청 객체가 다음 페이지까지 유효하다
	...
	public void count(int bnum) {
	 ...
	 public String temp(RedirectAttributes rttr) {
		 // redirect로 이동했을때 마지막으로 이동하는 페이지에
		 // 수집한 데이터를 전송하고 싶으면 rttr 사용가능
		 rttr.addAttribute("temp", "temp");
		 // rttr의 경우에는 Model interface를 구현하고 있기때문에
		 // 2번째고 저장할수있는 매개변수는 Object라고 작성은 되어있으나
		 // 문자열만 저장 가능하다
		 // (url에 queryString 형식으로 전송되는거라 한계가 있다 ㅠ)
